...
event = {'resource': '/connect', 'path': '/connect', 'httpMethod': 'POST', 'headers': None, 'multiValueHeaders': None, 'queryStringParameters': {'user_mail': 'ronialon2008@gmail.com'}, 'multiValueQueryStringParameters': {'user_mail': ['ronialon2008@gmail.com']}, 'pathParameters': None, 'stageVariables': None, 'requestContext': {'resourceId': 'v03ks6', 'resourcePath': '/connect', 'httpMethod': 'POST', 'extendedRequestId': 'Fv2aSEv1IAMF-Dw=', 'requestTime': '30/May/2023:17:50:44 +0000', 'path': '/connect', 'accountId': '339030231570', 'protocol': 'HTTP/1.1', 'stage': 'test-invoke-stage', 'domainPrefix': 'testPrefix', 'requestTimeEpoch': 1685469044735, 'requestId': 'be9211b9-dcc2-4195-bb42-7f60d5d62ffd', 'identity': {'cognitoIdentityPoolId': None, 'cognitoIdentityId': None, 'apiKey': 'test-invoke-api-key', 'principalOrgId': None, 'cognitoAuthenticationType': None, 'userArn': 'arn:aws:iam::339030231570:root', 'apiKeyId': 'test-invoke-api-key-id', 'userAgent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/113.0.0.0 Safari/537.36', 'accountId': '339030231570', 'caller': '339030231570', 'sourceIp': 'test-invoke-source-ip', 'accessKey': 'ASIAU536GZIJOPPXE6NY', 'cognitoAuthenticationProvider': None, 'user': '339030231570'}, 'domainName': 'testPrefix.testDomainName', 'apiId': 'aej45saso5'}, 'body': '{\n"user_to_connect":"matan.neyman@gmail.com"\n}\n', 'isBase64Encoded': False}
    ...
    try:
        if event.get("body") and user:
            ...
            if user_to_connect:
                ...
                logger.info(f"update connection from user {logged_user_data['user_email']} to {user_to_connect['user_email']}")
                update_connection(user_to_connect, logged_user_data['user_email'])
                logger.info(f"update connection from user {user_to_connect['user_email']} to {logged_user_data['user_email']}")
...
def update_connection(user_data, user_to_connect):
    ...
    recent_connections.append(connection_data)
    sorted_connection_list = sorted(recent_connections,
