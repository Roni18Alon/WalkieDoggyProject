/* eslint-disable jsx-a11y/anchor-is-valid */
/* eslint-disable jsx-a11y/alt-text */
// import User from "./images/kindpng_248729.png";
// import { Link } from "react-router-dom";

function OwnerHistoryContent() {
  return (
    <>
      <div
        className="col-12 col-md-9 col-md-9-profile p-4"
        style={{
          overflowY: "auto",
          position: "relative",
          overflowX: "hidden",
          height: "100vh",
          marginTop: "80px",
          backgroundColor: "#e2e8f0",
        }}
      >
        <div className="container bootdey">
          <div className="panel panel-default panel-order">
            <div className="panel-body">
              <div className="row">
                <div className="col-md-2">
                  <img
                    src="https://bootdey.com/img/Content/user_3.jpg"
                    className="media-object img-thumbnail"
                  />
                </div>
                <div className="col-md-10">
                  <div className="row">
                    <div className="">
                      <span>
                        <strong>Order name</strong>
                      </span>{" "}
                      <span className="label label-info">group name</span>
                      <br />
                      <span>
                        <strong>Status</strong>
                      </span>{" "}
                      <span className="label label-info">Completed</span>
                      <br />
                      cost: $323.13 <br />
                    </div>
                    <div className="col-md-12">
                      order made on: 05/31/2014 23:40 by{" "}
                      <a href="#">Jane Doe </a>
                    </div>
                  </div>
                </div>
              </div>
              <div className="row mt-4">
                <div className="col-md-2">
                  <img
                    src="https://bootdey.com/img/Content/user_1.jpg"
                    className="media-object img-thumbnail"
                  />
                </div>
                <div className="col-md-10">
                  <div className="row">
                    <div className="col-md-12">
                      <span>
                        <strong>Order name</strong>
                      </span>{" "}
                      <span className="label label-info">group name</span>
                      <br />
                      <span>
                        <strong>Status</strong>
                      </span>{" "}
                      <span className="label label-info">Pending</span>
                      <br />
                      Cost: $12623.13
                      <br />
                    </div>
                    <div className="col-md-12">
                      order made on: 06/12/2014 22:33 by{" "}
                      <a href="#">Jane Doe </a>
                    </div>
                  </div>
                </div>
              </div>
              <div className="row mt-4">
                <div className="col-md-2">
                  <img
                    src="https://bootdey.com/img/Content/user_3.jpg"
                    className="media-object img-thumbnail"
                  />
                </div>
                <div className="col-md-10">
                  <div className="row">
                    <div className="">
                      <span>
                        <strong>Order name</strong>
                      </span>{" "}
                      <span className="label label-info">group name</span>
                      <br />
                      <span>
                        <strong>Status</strong>
                      </span>{" "}
                      <span className="label label-info">Completed</span>
                      <br />
                      cost: $323.13 <br />
                    </div>
                    <div className="col-md-12">
                      order made on: 05/31/2014 21:10 by{" "}
                      <a href="#">Jane Doe </a>
                    </div>
                  </div>
                </div>
              </div>
              <div className="row mt-4">
                <div className="col-md-2">
                  <img
                    src="https://bootdey.com/img/Content/user_1.jpg"
                    className="media-object img-thumbnail"
                  />
                </div>
                <div className="col-md-10">
                  <div className="row">
                    <div className="col-md-12">
                      <span>
                        <strong>Order name</strong>
                      </span>{" "}
                      <span className="label label-info">group name</span>
                      <br />
                      <span>
                        <strong>Status</strong>
                      </span>{" "}
                      <span className="label label-info">Pending</span>
                      <br />
                      Cost: $12623.13
                      <br />
                    </div>
                    <div className="col-md-12">
                      order made on: 06/12/2014 09:32 by{" "}
                      <a href="#">Jane Doe </a>
                    </div>
                  </div>
                </div>
              </div>
              <div className="row mt-4">
                <div className="col-md-2">
                  <img
                    src="https://bootdey.com/img/Content/user_3.jpg"
                    className="media-object img-thumbnail"
                  />
                </div>
                <div className="col-md-10">
                  <div className="row">
                    <div className="">
                      <span>
                        <strong>Order name</strong>
                      </span>{" "}
                      <span className="label label-info">group name</span>
                      <br />
                      <span>
                        <strong>Status</strong>
                      </span>{" "}
                      <span className="label label-info">Completed</span>
                      <br />
                      cost: $323.13 <br />
                    </div>
                    <div className="col-md-12">
                      order made on: 05/31/2014 11:56 by{" "}
                      <a href="#">Jane Doe </a>
                    </div>
                  </div>
                </div>
              </div>
              <div className="row mt-4">
                <div className="col-md-2">
                  <img
                    src="https://bootdey.com/img/Content/user_3.jpg"
                    className="media-object img-thumbnail"
                  />
                </div>
                <div className="col-md-10">
                  <div className="row">
                    <div className="">
                      <span>
                        <strong>Order name</strong>
                      </span>{" "}
                      <span className="label label-info">group name</span>
                      <br />
                      <span>
                        <strong>Status</strong>
                      </span>{" "}
                      <span className="label label-info">Completed</span>
                      <br />
                      cost: $323.13 <br />
                    </div>
                    <div className="col-md-12">
                      order made on: 05/31/2014 15:15 by{" "}
                      <a href="#">Jane Doe </a>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </>
  );
}

export default OwnerHistoryContent;
